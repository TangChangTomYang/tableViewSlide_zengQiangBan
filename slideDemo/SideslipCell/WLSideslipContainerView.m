...
@interface WLSideslipContainerView()

@property (nonatomic) NSMutableArray<NSNumber *> *originWidths;
 //这里，由于外部有可能会追加一些确认删除等等按钮。不能单独判断self.subViews
@property (nonatomic,readwrite) NSArray<UIView *> *originSubViews;
@property (nonatomic,readwrite) NSArray<UIButton *> *subButtons;
@property (nonatomic) CGFloat currentSubViewHeight; 
@end


@implementation WLSideslipContainerView
...
- (void)layoutSubviews {
    ...
    if (self.frame.size.height != _currentSubViewHeight) {
        ...
        for (UIView *btnBgView in self.subviews) {
            //同步一下新的高度
            btnBgView.frame = CGRectMake(btnBgView.frame.origin.x,
                                         btnBgView.frame.origin.y,
                                         btnBgView.frame.size.width,
                                         self.frame.size.height);
            
            if (btnBgView.subviews.firstObject) {
                btnBgView.subviews.firstObject.frame = CGRectMake(0,
                                                                  btnBgView.subviews.firstObject.frame.origin.y,
                                                                  btnBgView.subviews.firstObject.frame.size.width,
                                                                  self.frame.size.height);
            }
        }
...
- (instancetype)initWithActions:(NSArray<WLSideslipCellAction *> *)actions {
    ...
    if (self) {
        for (int i = 0; i < actions.count; i++) {
            ...
            WLSideslipCellAction *action = actions[i];
            ...
            btnBgView.frame = CGRectMake(i?[_originWidths[i-1] floatValue]:0,
                                         0,
                                         width + action.margin*2,
                                         0);
